$("#queryCategoryLevel1").change(function(){
	...
	if(queryCategoryLevel1 != '' && queryCategoryLevel1 != null){
		$.get({
			url:"queryCategoryLevel1/"+queryCategoryLevel1,//请求的url
			// type:"get",//请求类型
...
$("#queryCategoryLevel2").change(function(){
	...
	if(queryCategoryLevel2 != '' && queryCategoryLevel2 != null){
		$.ajax({
			type:"GET",//请求类型
			url:"queryCategoryLevel1/"+queryCategoryLevel2,//请求的url
...
$(".addVersion").on("click",function(){
	var obj = $(this);
	// window.location.href="/app/appversionAdd/"+obj.attr("appinfoid");
});
$(".modifyVersion").on("click",function(){
	...
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		if(versionid == null || versionid == ""){
			...
			window.location.href="/app/appversionmodify/"+appinfoid;
		}
	}else{
...
$(".modifyAppInfo").on("click",function(){
	...
	var status = obj.attr("status");
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		window.location.href="/app/appversionmodify/"+ obj.attr("appinfoid");
	}else{
...
// 	var obj = $(this);
// 	window.location.href="/app/appview/"+ obj.attr("appinfoid");
// });
...
$(".deleteApp").on("click",function(){
	...
	if(confirm("你确定要删除APP应用【"+obj.attr("appsoftwarename")+"】及其所有的版本吗？")){
		$.ajax({
			type:"GET",
			url:"deleteAll/"+obj.attr("appinfoid"),
